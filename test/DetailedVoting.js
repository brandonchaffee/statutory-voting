...
const DVoting = artifacts.require('./DetailedVoting.sol')
...
contract('Detailed Voting', function (accounts) {
  beforeEach(async function () {
    await advanceBlock()
    this.midTime = latestTime() + duration.minutes(10)
    this.endTime = latestTime() + duration.days(1)
    this.token = await DVoting.new(votingWindow, supply, {from: accounts[0]})
  })
  //     it('emits detail hash', async function(){
  //
  //     })
  // })
